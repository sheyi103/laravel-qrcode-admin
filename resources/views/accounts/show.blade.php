...
@if (Auth::user()->id == $account->user_id && $account->applied_for_payout != 1)
        ...
        {!! Form::open(['route' => ['accounts.apply_for_payout'], 'method' => 'post', 'class'=>'pull-left']) !!}
            <input type="hidden" value="{{$account->id}}" name="apply_for_payout"/>
             ...
             {!! Form::button('<i class="fas fa-credit-card"></i> Apply for Payout', ['type' => 'submit', 'class' => 'btn btn-primary ', 'onclick' => "return confirm('Are you sure you wish to apply for payout?')"]) !!}
...
@if(Auth::user()->role_id < 3 && $account->paid !=1)
         ...
         <input type="hidden" value="{{$account->id}}" name="mark_as_paid"/>
             ...
             {!! Form::button('<i class="fas fa-credit-card"></i> Mark as Paid', ['type' => 'submit', 'class' => 'btn btn-primary ', 'onclick' => "return confirm('Are you sure you wish to confirm payout?')"]) !!}
